...
device = "cpu"
LR_nodes = pd.read_csv("../../data/LR_database/intercell_nodes_updated.csv",index_col=0)
Omnipath_network = pd.read_csv("../../data/LR_database/intercell_interactions.csv",index_col=0)
LR_nodes.index = LR_nodes["Id"].tolist()
# 在分配之前打印 "Omnipath_network" 数据框的 'Src' 列和 'LR_nodes' 索引的长度
print("分配之前（Src 列）：")
print("Omnipath_network 中 'Src' 列的长度：", len(Omnipath_network["Src"].tolist()))
print("LR_nodes 索引的长度：", len(LR_nodes))
...
for i in range(len(df_list)):
    ...
    temp = temp.head(1000)
    temp.to_csv(f"../../results/GSE133079/intercell_Mouse_{i+1}_random.csv")
